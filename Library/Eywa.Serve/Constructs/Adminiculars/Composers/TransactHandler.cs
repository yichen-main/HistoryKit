...
public abstract class TransactHandler<T1> : IRequestHandler<T1> where T1 : IRequest
{
    ...
    {
        Configure();
        await DurableSetup.TransactionAsync(async (connection, transaction, reader) => await HandleAsync(new()
        {
            Import = import,
...
public abstract class TransactHandler<T1, T2> : IRequestHandler<T1, T2> where T1 : IRequest<T2> where T2 : notnull
{
    protected readonly struct Options
    {
        public required T1 Import { get; init; }
